...
class MqttClientManager(
    ...
    fun reconnectOnStart(context: Context) {
        CoroutineScope(Dispatchers.Main).launch {
            // Set up MQTT connection options (customize as needed)
            val options = MqttConnectOptions()
            options.isAutomaticReconnect = true
            ...
            try {
                ...
                Toast.makeText(context, "Successfully to MQTT broker", Toast.LENGTH_SHORT).show()
                // Perform actions after successful reconnection
            } catch (e: MqttException) {
                Toast.makeText(context, "Failed to reconnect to MQTT broker", Toast.LENGTH_SHORT).show()
                // Handle reconnection failure
            }
        }
    }
